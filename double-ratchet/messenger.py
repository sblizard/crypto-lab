...
class MessageHeader:
    ...
    def deserialize(data: bytes) -> "MessageHeader":
        ...
        try:
            ...
            raise Exception("Invalid DH public key")
...
class MessengerServer:
    ...
    def signCert(self, cert: Certificate) -> bytes:
        name = cert.getUserName().encode("utf-8")
...
class MessengerClient:
    ...
    def receiveCertificate(self, certificate: Certificate, signature: bytes) -> None:
        try:
            name_bytes = certificate.getUserName().encode("utf-8")
            pk_bytes = certificate.getPublicKey().public_bytes(
                encoding=serialization.Encoding.X962,
                format=serialization.PublicFormat.UncompressedPoint,
            )
            ...
            raise Exception(f"certificate verification failed: {e}")
